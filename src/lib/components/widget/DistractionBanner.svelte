<script lang="ts">
  ...
  let { label, seconds, activeTitle }: Props = $props();
</script>

<!-- Kept separate so warning copy can evolve independently from creature forms. -->
<div class="warning-caption" role="alert" aria-live="polite">
  <strong>{label}</strong>
  <span>{seconds}s · {activeTitle}</span>
</div>
